...
(function () {
    ...
    api('/api/admin/overview').then(function (d) {
      ...
      body.innerHTML =
        ...
        d.teachers.map(function (t) {
          ...
          return '<tr><td>' + esc(t.name) + '</td><td><code dir="ltr">/' + esc(t.slug) + '</code></td><td>' +
      ...
      body.innerHTML =
        ...
        (d.archived && d.archived.length ? '<div class="section-title" style="margin-top:22px"><h3>الأرشيف (محذوفون — قابلون للاستعادة)</h3></div>' +
          ...
          d.archived.map(function (t) {
            return '<tr><td>' + esc(t.name) + '</td><td><code dir="ltr">/' + esc(t.slug) + '</code></td><td style="font-size:.76rem">' + (t.archivedAt ? new Date(t.archivedAt).toLocaleString('ar-EG') : '—') + '</td>' +
  ...
  function readImportFile(file) {
    ...
    r.onload = function () {
      ...
      var n = $('impName'); n.style.display = ''; n.textContent = file.name + ' · ' + fmtSize(file.size);
      ...
      body.innerHTML =
        ...
        d.results.map(function (r) {
          ...
          return '<tr><td style="font-size:.74rem">' + new Date(r.date).toLocaleString('ar-EG') + '</td>' +
            ...
            '<td><code dir="ltr">/' + esc(r.teacherSlug || '') + '</code></td></tr>';
